...
public class AES {
    ...
    public static byte[] encrypt(byte[] plainText, byte[] key) {
        ...
        for (int i = 0; i < cipherBytes.length; i += 16) {
            encryptedBlock = encryption(key, copyOfRange(plainText, i, i + 16));
            System.arraycopy(getByteArray(encryptedBlock), 0, cipherBytes, i, 16);
        }
        return cipherBytes;
    }
    ...
    public static byte[] decrypt(byte[] cipherBytes, byte[] key) {
        ...
        for (int i = 0; i < decryptedBytes.length; i += 16) {
            decryptedBlock = decryption(key, copyOfRange(cipherBytes, i, i + 16));
            System.arraycopy(getByteArray(decryptedBlock), 0, decryptedBytes, i, 16);
        }
        int lastIndex = 0;
        while (lastIndex != -1) {
    ...
    /**
     * this method combines the columns with themselves using galoisMultiply.
     *
     * @param state a 2d array that hold the data that is being encrypted
     */
    ...
    /**
     * this method combines the columns with the inverse of themselves using galoisMultiply.
     *
     * @param state a 2d array that hold the data that is being encrypted
     */
    ...
    /**
     * preforms galois Multiplication on int a and int b
     * @param a an int
     * @param b and int
     * @return an int, the result of galois Multiplication on int a and int b
     */
    ...
    /** it converts a 16 byte array into a 2d 4x4 byte array in [column][row] format
    ...
    /** it takes in a plaintext byte array, loads it into a 128 bit block state, generates round keys using getRoundKey,
     * adds the 1st round key to the state at round 0
     * substitutes bytes, shifts rows, mixes columns and adds the corresponding round key with its respective methods for rounds 1-9
     * substitutes bytes, shifts rows, and adds the corresponding round key with its respective methods for the last round
     * lastly it returns the state
     * @param key a length 16(128 bits) byte array that contains the key to encrypt the plaintext
     * @param plaintext a length 16(128 bits) byte array the plaintext as a byte array
     * @return a 4x4 128-bit encrypted byte array
     */
    ...
    /** it takes in a  ciphertext byte array, loads it into a 128 bit block state, generates round keys using getRoundKey,
     * adds round key #10, inverse shift rows, and inverse substituted bytes of state using respective methods.
     * adds corresponding round key, inverse mixes columns, inverse shifts rows, and substitutes inverse bytes of state using respective method for rounds 9-1
     * adds round key #0 to state
     * lastly it returns the decrypted state
     * @param key a length 16(128 bits) byte array that contains the key to decrypt the plaintext
     * @param ciphertext a length 16(128 bits) byte array, the ciphertext as a byte array
     * @return a 4x4 128-bit encrypted byte array
     */
    ...
    /**
     * this method rotates the rows to the right in a specific way.
     * Row 0 is rotated to the right 0 bytes, Row 1 is rotated to the right 1 byte, Row 2 is rotated to the right 2 bytes, Row 3 is rotated to the right 3 bytes.
     *
     * @param state a 2d array that hold the data that is being encrypted
     */
    ...
    /**
     * this method substitutes the bytes in the state with bytes from the inverse S-Box
     *
     * @param state a 2d array that hold the data that is being decrypted
     */
